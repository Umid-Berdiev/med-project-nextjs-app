...
export default function AppDropdown({
  ...
  return (
    <div className={`dropdown font-semibold`} ref={appDropdownRef}>
      <div
        className={`hover:bg-secondary-light btn btn-sm h-9 rounded-lg border-info text-info hover:border-info ${isDropdownOpen ? 'bg-secondary-light' : 'bg-white'}`}
        role='button'
        tabIndex={0}
        ...
        <div className='dropdown-content z-[1] rounded-box bg-base-100 p-2 shadow'>
          ...
          <ul tabIndex={0} className='menu px-0'>
            {options.map((option, index) => (
              <li key={index}>
                {typeof option === 'function' ? option() : <a>{option.name}</a>}
              </li>
            ))}
          </ul>
        </div>
